...
**Status:** early stage. So far only the file *loader* is implemented — it parses a numeric CSV file into a 2D NumPy array. The linear algebra commands listed under [Feature](#feature) are still a roadmap.
...
```sh
python -m lacli.main -f path/to/file
```
...
Current flags: `-f/--file` (required, the file to load) and `-b` (enable a simple start-to-end benchmark). Parsing parallelism is handled internally by PyArrow.
