...
**Status:** sections [0]–[4] of the [Feature](#feature) roadmap are implemented and reachable from the CLI — basic matrix operations, property checks, rotations, factorizations and least squares, all backed by NumPy in [`src/la_cli/core`](src/la_cli/core). I/O beyond headerless CSV (section [5]) is still a roadmap.
...
python -m la_cli.main bmo matmul -f A.csv -g B.csv

# LU decomposition, written to CSV
python -m la_cli.main factorization lu -f A.csv -out lu.csv

# ridge least squares (lambda via -k)
python -m la_cli.main least-squares regularized -f A.csv -g b.csv -k 1.0
```

Use `python -m la_cli.main <command> <feature> --help` to list a feature's flags.
