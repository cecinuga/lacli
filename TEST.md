...
`tests/` covers the file-loading path end to end: `lacli.main.run` must turn a CSV
...
`test_load.py` defines a single test, `test_lacli`, marked `@pytest.mark.integration`.
...
- `shared_fd` — opens the current `dataset_path` read-only and yields its file
  descriptor (this is what `lacli.main.run` actually consumes).
...
dataset file is run through `lacli.main.run` once per thread count. This is
